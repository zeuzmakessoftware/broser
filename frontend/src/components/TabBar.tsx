...
export function TabBar({ tabs, onNewTab, onSwitchTab, onCloseTab, className }: TabBarProps) {
  return (
    <div className={twMerge("flex items-end h-10 px-2 gap-1 no-drag-region", className)}>
      {tabs.map(tab => (
        <div
          key={tab.id}
          onClick={() => onSwitchTab(tab.id)}
          className={clsx(
            "group relative flex items-center min-w-[120px] max-w-[240px] h-9 px-3 rounded-t-lg cursor-default transition-colors duration-200 select-none",
            tab.active 
